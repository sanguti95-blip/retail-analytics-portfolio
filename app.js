...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderPLTable(records, totalVentas) {
        ...
        mainCategoryKeys.forEach(catKey => {
            ...
            tr.innerHTML = `
                <td>${expandBtnHtml} ${exactKey || catKey}</td>
            ...
            if ((catKey === 'SUB-TOTAL GASTOS DE OPERACIÓN' || catKey === 'TOTAL GASTOS DE OPERACIÓN') && state.expandedPLAccounts.has(catKey)) {
                ...
                detailAccounts.forEach(childName => {
                    ...
                    if (childVal !== 0) {
                        ...
                        childTr.innerHTML = `
                            <td class="pl-row-sub">${childName}</td>
    ...
    function renderExpensesComparison(records, yearA, yearB, maxMonthB) {
        ...
        const defaultExpenses = [
            'Planilla', 'Alquileres', 'Servicios Publicos', 'Marketing', 
            'Reparaciones y Mantenimientos', 'Flotilla Vehicular', 
            'Gastos Corporativos', 'Gastos Financieros', 'Otros Gastos', 'Staff Gerencial'
        ];
        ...
        if (!state.selectedExpenseAccounts) {
            ...
            allAccounts.forEach(acc => {
                if (defaultExpenses.some(d => acc.toLowerCase().includes(d.toLowerCase()))) {
        ...
        if (tbody) {
            ...
            expArray.forEach(e => {
                ...
                let tds = `<td><strong style="white-space: nowrap;">${e.acc}</strong></td>`;
                let fullYearTotal = 0;
                for(let i=0; i <= maxMonthB; i++) {
    ...
    function renderStockSubSection() {
        ...
        items.forEach(item => {
            ...
            tr.innerHTML = `
                ...
                <td class="text-bold">${item.articulo}</td>
